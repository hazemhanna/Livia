...
extension WalletVc: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? RefundOrderCell else { return UITableViewCell()}
            ...
            cell.config(imagePath: list[indexPath.row].orderDetail?[0].meal?.image ?? "", desc: list[indexPath.row].orderDetail?[0].meal?.nameAr ?? "", name:  list[indexPath.row].orderDetail?[0].meal?.nameAr ?? "" , date:  list[indexPath.row].createdAt ?? "")
            }else{
                cell.config(imagePath: list[indexPath.row].orderDetail?[0].meal?.image ?? "", desc: list[indexPath.row].orderDetail?[0].meal?.nameEn ?? "", name:  list[indexPath.row].orderDetail?[0].meal?.nameEn ?? "" , date:  list[indexPath.row].createdAt ?? "")
            }
            cell.goToDetails = {
        ...
        return cell
            ...
            if "lang".localized == "ar" {
            cell.config(date: self.rewards[indexPath.row].createdAt ?? "" , title: self.rewards[indexPath.row].titleAr ?? "", value: self.rewards[indexPath.row].value ?? 0)
            }else{
                cell.config(date: self.rewards[indexPath.row].createdAt ?? "" , title: self.rewards[indexPath.row].titleEn ?? "", value: self.rewards[indexPath.row].value ?? 0)
            }
            return cell
        }
    }
