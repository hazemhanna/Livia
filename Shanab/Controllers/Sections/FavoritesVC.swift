    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
     ...
     //   cell.price.isHidden = true
        ...
        if "lang".localized == "ar" {
            cell.config(name: item_type.nameAr ?? "", time: String(item_type.deliveryTime ?? 0), rate: Double(item_type.rate ?? 0) , price: Double(item_type.deliveryFee ?? 0), imagePath: item_type.image ?? "", type: item_type.type ?? "")
        } else {
            cell.config(name: item_type.nameEn ?? "", time: String(item_type.deliveryTime ?? 0), rate: Double(item_type.rate ?? 0) , price: Double(item_type.deliveryFee ?? 0), imagePath: item_type.image ?? "", type: item_type.type ?? "")
        }
